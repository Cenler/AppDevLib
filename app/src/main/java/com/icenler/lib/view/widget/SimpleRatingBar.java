...
public class SimpleRatingBar extends FrameLayout implements View.OnClickListener {
    ...
    static final int DEF_MAX_COUNT = 5;

    /**
     * Max count of star
     */
    private int mMaxCount = DEF_MAX_COUNT;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs == null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                mFillDrawable = context.getDrawable(android.R.drawable.star_big_on);
                mEmptyDrawable = context.getDrawable(android.R.drawable.star_big_off);
            } else {
                mFillDrawable = context.getResources().getDrawable(android.R.drawable.star_big_on);
                mEmptyDrawable = context.getResources().getDrawable(android.R.drawable.star_big_off);
            }
        } else {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SimpleRatingBar);
            mMaxCount = typedArray.getInteger(R.styleable.SimpleRatingBar_srb_max_count, mMaxCount);
            mCount = typedArray.getInteger(R.styleable.SimpleRatingBar_srb_count, 0);
            mFillDrawable = typedArray.getDrawable(R.styleable.SimpleRatingBar_srb_fill);
            mEmptyDrawable = typedArray.getDrawable(R.styleable.SimpleRatingBar_srb_empty);
            mSpace = typedArray.getDimensionPixelSize(R.styleable.SimpleRatingBar_srb_space, 0);
            mClickRating = typedArray.getBoolean(R.styleable.SimpleRatingBar_srb_click_rating, true);
            mTouchRating = typedArray.getBoolean(R.styleable.SimpleRatingBar_srb_touch_rating, true);
            typedArray.recycle();
    ...
    public interface OnRatingChangeListener {
         ...
         */
        void onChange(SimpleRatingBar ratingBar, int preCount, int curCount);
    }
